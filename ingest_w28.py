"""Ingest W28 (6-12 Jul 2026) — all roles."""
import sys, json, csv
...
n_read = 0; roles_seen = defaultdict(int); dates_seen = defaultdict(int)

with open(f'{UPLOADS}/week 28.csv', encoding='utf-8-sig') as f:
    r = csv.reader(f)
    header = next(r)
    ...
    for row in r:
        ...
        try: dt = datetime.strptime(date_raw, '%d %b %Y')
        except: continue
...
print(f"Dates: {dict(sorted(dates_seen.items()))}")
